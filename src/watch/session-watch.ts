...
import { getCachePath, getRepoCachePath } from '../cache.js'
import { shallowClone, updateShallowClone, normalizeGitUrl } from '../clone/hardened.js'
...
export async function createWatchSession(input: WatchSessionInput): Promise<WatchSessionResult> {
  ...
  const driftResult = await detectDrift(
    ...
    input.oldSha,
    input.newSha,
    input.trackedPaths.map(path => ({ kind: input.targetKind, path }))
  )
